...
private:
    ...
    static const size_t BOAT_CAPACITY = 2;

    //GAME CONSTANTS
    const char PRINT = 'p';
    const char BOARD = 'e';
    const char UNBOARD = 'd';
    const char MOVE_BOAT = 'm';
...
public:
    ...
    static const size_t getBoatCapacity();
...
private:
     ...
     */
    bool checkConstraints(const Container& container, const Person* person);


    /***
     * Board/Unboard a person
     * @param person the person
     * @param board if the person is either boarding or unboarding
     */
    void move(const Person* person, bool board);
};
